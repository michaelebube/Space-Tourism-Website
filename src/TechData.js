import LaunchImgPortrait from './assets/technology/image-launch-vehicle-portrait.jpg'
import LaunchImgLandscape from './assets/technology/image-launch-vehicle-landscape.jpg'
import SpacePortImgPortrait from './assets/technology/image-spaceport-portrait.jpg'
import SpacePortImgLandscape from './assets/technology/image-spaceport-landscape.jpg'
import SpaceCapsuleImgPortrait from './assets/technology/image-space-capsule-portrait.jpg'
import SpaceCapsuleImgLandscape from './assets/technology/image-space-capsule-landscape.jpg'
import LaunchImgTablet from './assets/technology/image-launch-vehicle-tablet.jpg'
import SpacePortImgTablet from './assets/technology/image-spaceport-tablet.png'
import SpaceCapsuleImgTablet from './assets/technology/image-space-capsule-tablet.jpg'
import LaunchImgMobile from './assets/technology/image-launch-vehicle-mobile.png'
import SpacePortImgMobile from './assets/technology/image-spaceport-mobile.png'
import SpaceCapsuleImgMobile from './assets/technology/image-space-capsule-mobile.png'



const TechData = {
    'Launch' : {
        imgPortrait: LaunchImgPortrait,
        imgLandscape: LaunchImgLandscape,
         imgTablet: LaunchImgTablet ,
         imgMobile: LaunchImgMobile,
        title: 'LAUNCH VEHICLE',
        content: "A launch vehicle or carrier rocket is a rocket-propelled vehicle used to carry a payload from Earth's surface to space, usually to Earth orbit or beyond. Our WEB-X carrier rocket is the most powerful in operation. Standing 150 metres tall, it's quite an awe-inspiring sight on the launch pad!"
    },
    'SpacePort': {
        imgPortrait: SpacePortImgPortrait,
        imgLandscape: SpacePortImgLandscape,
        imgTablet: SpacePortImgTablet ,
        imgMobile: SpacePortImgMobile,
        title: 'SPACEPORT',
        content: "A spaceport or cosmodrome is a site for launching (or receiving) spacecraft, by analogy to the seaport for ships or airport for aircraft. Based in the famous Cape Canaveral, our spaceport is ideally situated to take advantage of the Earth's rotation for launch."
    },
    'SpaceCapsule': {
        imgPortrait: SpaceCapsuleImgPortrait ,
        imgLandscape: SpaceCapsuleImgLandscape,
         imgTablet: SpaceCapsuleImgTablet,
         imgMobile: SpaceCapsuleImgMobile,
        title: 'SPACE CAPSULE',
        content: "A space capsule is an often-crewed spacecraft that uses a blunt-body reentry capsule to reenter the Earth's atmosphere without wings. Our capsule is where you'll spend your time during the flight. It includes a space gym, cinema, and plenty of other activities to keep you entertained."
    }
}

export default TechData;